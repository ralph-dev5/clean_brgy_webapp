<aside class="w-64 bg-white shadow-lg h-screen sticky top-0">
    <div class="p-6">
        ...
        <nav class="flex flex-col gap-2">
            ...
            <!-- Paste the Trash link here -->
            <a href="{{ route('admin.reports.trash') }}" class="text-gray-700 hover:bg-gray-200 px-4 py-2 rounded transition">
                🗑 Trash
            </a>

            <form method="POST" action="{{ route('logout') }}" class="mt-4">
                @csrf
                <button type="submit" class="w-full text-left text-red-600 hover:bg-red-100 px-4 py-2 rounded transition">
                    Logout
                </button>
            </form>
        </nav>
