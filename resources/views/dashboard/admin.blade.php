    ...
    <aside class="w-64 h-screen bg-green-700 text-white flex flex-col justify-between fixed">
        ...
        <div>
            ...
            <nav class="mt-6 flex flex-col space-y-2 px-4">
                <a href="{{ route('admin.dashboard') }}" 
                   class="px-4 py-2 rounded-lg hover:bg-green-600 transition">
                    Dashboard
                </a>
        ...
        <div class="p-4 border-t border-green-600">
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button class="w-full px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition flex items-center justify-center gap-2">
                    <span>Log Out</span>
                </button>
            </form>
        </div>
